...
class DataReg(cleanData):
    ...
    def semipar_data(self):
        ...
        if (
            "munitable" not in self.conn.list_tables()
            or "zipstable" not in self.conn.list_tables()
        ):
        ...
        return (
            df_qcew  # .join(df_dp03, predicates=["year", "qtr", "zipcode"], how="left")
        )
    ...
    def make_spatial_table(self):
        if not os.path.exists(f"{self.saving_dir}external/zip_shape.zip"):
            self.pull_file(
                ...
                filename=f"{self.saving_dir}external/zip_shape.zip",
            )
        ...
        gdf = gdf[gdf["STATEFP"].str.startswith("72")].rename(
            columns={"geometry": "count_geo"}
        )
        ...
        for geo in gdf["GEOID"].values:
            ...
            tmp["GEOID"] = geo
            master_df = pd.concat(
                [master_df, tmp[["ZCTA5CE20", "GEOID"]]], ignore_index=True
            )
        master_df = pd.merge(
            zips,
            master_df.drop_duplicates(subset=["ZCTA5CE20"]),
            on="ZCTA5CE20",
            validate="1:1",
        )
        master_df = pd.merge(master_df, gdf, on="GEOID", validate="m:1")
        master_df = master_df[["GEOID", "ZCTA5CE20"]]
        master_df = master_df.rename(
            columns={"GEOID": "geo_id", "ZCTA5CE20": "zipcode"}
        )
        gdf = (
            gdf[["GEOID", "NAME"]]
            .rename(columns={"GEOID": "geo_id", "NAME": "municipality"})
